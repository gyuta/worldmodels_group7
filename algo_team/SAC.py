...
from algo_team.Replay_Buffer import Replay_Buffer
...
class SAC(Base_Agent):
    ...
    def produce_action_and_action_info(self, state):
        ...
        actor_output = self.actor_local(state.to(self.device))
    ...
    def learn(self):
        ...
        qf1_loss, qf2_loss = self.calculate_critic_losses(state_batch.to(self.device), action_batch.to(self.device), reward_batch.to(self.device), next_state_batch.to(self.device), mask_batch.to(self.device))
        self.update_critic_parameters(qf1_loss, qf2_loss)

        policy_loss, log_pi = self.calculate_actor_loss(state_batch.to(self.device))
